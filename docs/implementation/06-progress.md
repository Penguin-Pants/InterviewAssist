...
| **M5 — Overlay UI** | 🟢 Everything buildable here is done · **T5.2 verified 2026-09-14** | T5.1, T5.2, T5.3, T5.4, T5.4a, T5.5, T5.6, T5.7, T5.8 complete and tested. `SetWindowDisplayAffinity` wired into `main_window.py`, feeding `Health.capture_excluded` through the existing monitor path. Remaining: **T5.9** (end-to-end latency, needs the D-U6 laptop) and the FR14 6-way manual matrix (real screen-share apps, not code) |
| **M6 — Session lifecycle** | 🟢 Logic complete · panic on hold (D-U11) | T6.1–T6.3, **T6.3b's panic surface**, T6.5 classification, T6.6 backpressure, T6.7 done. **T6.4's code built and wired 2026-09-14** — its 45-min ProcMon trace still needs a person. The OS trigger paths need Windows |
...
| **M11 — Post-interview report** | ✅ **Complete** — T11.2 verified 2026-09-14 | Record, evidence binding, encrypted store, retention, generation, the view/export, **context snapshots (D-58) and off-thread generation (D-59)**, and now the **DPAPI binding itself**, round-tripped for real on the Windows target. Nothing in M11 is outstanding |
...
| ~~**T11.2** — DPAPI cipher~~ | ✅ **Verified 2026-09-14** on the Windows target. `tests/test_win_dpapi.py` (new, marked `windows`) round-trips `CryptProtectData`/`CryptUnprotectData` for real, confirms encryption is not a pass-through, and confirms unrecognised ciphertext raises rather than decoding to nonsense — the same failure path a cross-account blob hits. `default_cipher()` returns `DpapiCipher` on this machine. | — |
| ~~**T0.5's OS half**~~ — Credential Manager | ✅ **Verified 2026-09-14.** `tests/test_win_credentials.py` (new, marked `windows`) confirms `_default_backend()` resolves to the real `keyring` vault (not the in-memory fallback) and round-trips a secret through it via `CredentialStore`. Runs against the actual shared Credential Manager, so it saves and restores whatever the probed account held before the test — which turned out to be a real stored `deepgram` key, correctly put back. | — |

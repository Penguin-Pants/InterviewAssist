...
| **T9.6a** Real dependency construction | FR43, D-U3, D-U12, D-U13 | `_build_application` constructs the embedder, model client, cipher and active note set. ✅ **Done 2026-09-14.** `python -m interview_prep_recall` starts; `notes/embedder.py` is the first concrete `Embedder` this codebase has ever had. Each dependency that cannot be satisfied degrades to a state preflight names rather than refusing to start: no model → empty index + `model_present` BLOCK; no key → `selector=None`, stage 1 only (D-U12); no user-bound cipher → `UnavailableCipher`, which refuses at the *write* so FR82 is kept by writing nothing. **Left behind:** the weights have never been downloaded here (**AS-10**, AS-9's neighbour, verify on the Windows machine); the download itself is T9.3's; a note set recovered during startup is not recorded on the ring, because the set must be loaded before the `Application` that owns the ring; **OQ-10** is answered only at its low end — the report is cloud-gated and says so, and a local evidence-only report is still open. *(Historical record of what it was blocked on follows.)* ~~**Deliberately unimplemented**, on two blockers — not three.~~ ~~FR43's active-note-set selection~~ **is done**: T3.8 writes the id and `ui.editor.load_active_set(root, default_settings())` reads it. ~~the **no-API-key policy**~~ **is also done: D-U12 (2026-09-09) decided local at startup, cloud keys optional**, so `client` becomes optional and `Stage2Selector` conditional on a key. What remains is the two dependencies themselves — the embedding model, blocked in this container by network policy rather than by platform, and DPAPI, which is genuinely Windows-only. Note the embedder is a *bigger* gap than "a model download": **no concrete `Embedder` implementation exists at all**, only the Protocol at `notes/index.py`, so matching returns no candidates even with a model present. **D-U13 (2026-09-09) settles how the models arrive** — first run downloads them, the user picks the Whisper model, the embedder is fixed and never a user choice, and the VAD ships bundled — so this task is now implementation with no open questions in front of it. It also gains a `model_present` BLOCK check in `session/preflight.py`. Raising is the honest state. |
